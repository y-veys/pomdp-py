...
cdef class POUCT(Planner):
    ...
    cpdef dict _update_beliefs(self, dict transition_beliefs, State state, Action action, State next_state, bint success):
        ...
        Beta posterior update rule:
        - If success (transitioned to target): (α, β) → (α+1, β)
        - If failure (self-loop): (α, β) → (α, β+1)

        Note: If (state, action, target_state) is not in beliefs (deterministic transition),
        returns beliefs unchanged.

        Args:
            beliefs (dict): Current beliefs {(state, action, target_state) -> (alpha, beta)}
        ...
        Returns:
            dict: New beliefs dictionary with updated parameters
        """
        ...
        alpha, beta = transition_beliefs[(state, action, next_state)]

        if success:
            new_transition_beliefs[(state, action, next_state)] = (alpha + 1, beta)
        else:
            new_transition_beliefs[(state, action, next_state)] = (alpha, beta + 1)
